...
for country, file_path in countries.items():
    ...
    # 进行Shapiro-Wilk正态性检验
    cat_stat, cat_p = stats.shapiro(cat_resid)
    dog_stat, dog_p = stats.shapiro(dog_resid)

    # 添加检验结果文本
    ax1.text(0.05, 0.95, f'Shapiro-Wilk test p-value: {cat_p:.4f}',
             transform=ax1.transAxes, verticalalignment='top')
    ax2.text(0.05, 0.95, f'Shapiro-Wilk test p-value: {dog_p:.4f}',
