...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[box,right=2cm of food] (pets) {Pet Population};
        \node[box,above right=0.8cm and 2cm of pets] (country) {Countries};
        \node[box,below right=0.8cm and 2cm of pets] (type) {Pet Types};
...
% 个数应与表的列数相同。此外，\verb|@{文本}|可以出现在任意两个上述的列格式之间，其中的文���将被插入每一行
